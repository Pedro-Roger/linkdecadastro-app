...
export default function CoursePage() {
  ...
  const fetchCourse = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.log('Curso recebido:', { id: data.id, title: data.title, bannerUrl: data.bannerUrl })
        setCourse(data)
  ...
  const handleShareCourse = useCallback(async () => {
    ...
    try {
      ...
      console.error('Erro ao compartilhar curso:', error)
    }
      ...
      <div className="bg-gray-100 py-2">
        <div className="container mx-auto px-4">
          <nav className="text-xs md:text-sm text-gray-600">
            <Link href="/courses" className="hover:text-[#FF6600]">Cursos</Link>
            {' / '}
            <Link href="/my-courses" className="hover:text-[#FF6600]">Meus Cursos</Link>
            {' / '}
