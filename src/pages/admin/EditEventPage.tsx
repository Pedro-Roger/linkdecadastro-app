...
export default function EditEventPage() {
    ...
    const fetchEventData = async () => {
        try {
            setLoadingData(true)
            // Tenta buscar dados do evento
            // Assumindo que o endpoint de lista retorna todos, ou existe um específico
            // Vamos tentar um get específico se existir, ou filtrar da lista se necessário
            // Mas o ideal é GET /admin/events/:id
            const data = await apiFetch(`/admin/events/${eventId}`, { auth: true })
            // Se falhar o endpoint específico, o catch pega.
            ...
            reset({
                ...
                status: data.status,
                maxRegistrations: (data.maxRegistrations ? String(data.maxRegistrations) : '') as any,
                slug: data.slug || ''
            })
            if (data.bannerUrl) {
